        ...
        <a-row :gutter="[15, 0]">
          ...
          <a-col :span="6">
            <a-form-item name="value" label="值">
              <a-input
                v-model:value="state.queryParams.value"
                placeholder="请输入值"
              />
            </a-form-item>
          </a-col>
          <a-col :span="6">
            <a-form-item name="remark" label="备注">
              <a-input
                v-model:value="state.queryParams.remark"
                placeholder="请输入备注"
              />
            </a-form-item>
          </a-col>
        ...
        <template #bodyCell="scope">
          <template v-if="scope?.column?.dataIndex === 'action'">
            <div flex gap-2>
              <a-button
                type="link"
                @click="handleEdit(scope?.record as CrudTableModel)"
              >
                编辑
              </a-button>
              <a-popconfirm
