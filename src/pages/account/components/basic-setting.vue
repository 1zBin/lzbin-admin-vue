...
const rules: any = computed(() => {
  return {
    ...
    region: [
      {
        required: true,
        message: t("account.settings.form-rule-region"),
        trigger: "change",
      },
    ],
    eamil: [
...
<template>
  <a-card :title="t('account.settings.basic-setting')" :bordered="false">
    <a-row class="row-box" justify="center">
      <a-col class="form-box" :span="16">
        <a-form
          ref="formRef"
          :model="formState"
          :rules="rules"
        ...
        >
          ...
          <a-form-item
            :label="t('account.settings.form-region')"
            :label-col="{ span: 24 }"
            name="region"
          >
            <a-select
