...
export const lsLocaleState = useStorage(
  LOCALE_KEY,
...
export const useI18nLocale = createSharedComposable(() => {
  ...
  const setLocale = async (locale: string) => {
    ...
    try {
      ...
      else (i18n.global.locale as any).value = locale as any;
      loading.value = false;
    } catch (e) {
      loading.value = false;
    }
  ...
  watch(
    locale,
    () => {
      if (antd.value && antd.value.locale) dayjs.locale(antd.value.locale);
  ...
  const t = (key: string, defaultMessage?: string) => {
    ...
    if (message !== key) return (i18n?.global as any)?.t?.(key);
